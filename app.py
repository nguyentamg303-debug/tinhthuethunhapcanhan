...
import pandas as pd
# ==============================================================================
# 1. CẤU HÌNH TRANG WEB VÀ GIAO DIỆN CHÍNH
# ==============================================================================
st.set_page_config(page_title="App Tính Thuế TNCN Việt Nam 2026", page_icon="💰",layout="centered")
# --- CHÈN LOGO THEO FILE TRỰC TIẾP ---
# Cơ chế an toàn tránh crash ứng dụng nếu thiếu file ảnh
try:
    st.image("logo.jpg")
except FileNotFoundError:
    st.warning("⚠️ Hệ thống không tìm thấy tệp tin 'logo.jpg'. Vui lòng kiểm tra lại đường dẫn thư mục.")
...
st.markdown("---")
# ==============================================================================
# 2. PHẦN NHẬP DỮ LIỆU ĐẦU VÀO từ người dùng
# ==============================================================================
...
st.markdown("---")
# ==============================================================================
# 3. LỚP LOGIC TÍNH TOÁN (CORE ENGINE)
# ==============================================================================
def tinh_thue_tncn(gross, bonus, overtime, lunch, other, deps):
    # Tổng thu nhập gộp phát sinh trong tháng
    total_income = gross + bonus + overtime + lunch + other
    # Tính các khoản bảo hiểm bắt buộc trích từ lương (Tổng 10.5%)
    bhxh = gross * 0.08
    bhyt = gross * 0.015
    bhtn = gross * 0.01
    total_insurance = bhxh + bhyt + bhtn
    # Định mức giảm trừ gia cảnh (Cập nhật mốc định mức của bạn năm 2026)
    self_reduction = 15500000  
    dependent_reduction = deps * 6200000  
    total_reduction = self_reduction + dependent_reduction
    # Xác định các khoản thu nhập được miễn thuế theo quy định
    exempt_lunch = min(lunch, 730000)
    exempt_allowance = other 
    total_exempt_income = overtime + exempt_lunch + exempt_allowance
    # Thu nhập tính thuế (Assessable Income) làm căn cứ đưa vào biểu lũy tiến
    assessable_income = max(0, total_income - total_exempt_income - total_insurance - total_reduction)
    # Cấu trúc biểu thuế lũy tiến từng phần 5 bậc năm 2026
    brackets = [{"limit": 10000000, "rate": 0.05, "desc": "Bậc 1: Đến 10 triệu đồng (5%)"},{"limit": 30000000, "rate": 0.10, "desc": "Bậc 2: Trên 10 đến 30 triệu đồng (10%)"},{"limit": 60000000, "rate": 0.20, "desc": "Bậc 3: Trên 30 đến 60 triệu đồng (20%)"},{"limit": 100000000, "rate": 0.30, "desc": "Bậc 4: Trên 60 đến 100 triệu đồng (30%)"},{"limit": float('inf'), "rate": 0.35, "desc": "Bậc 5: Trên 100 triệu đồng (35%)"}]
    tax = 0
    temp_income = assessable_income
    previous_limit = 0
    tax_breakdown = []
    # Thuật toán quét và bóc tách thu nhập theo từng bậc thuế lũy tiến
    for b in brackets:
        ...
        if temp_income > 0:
            ...
            break
    # Thu nhập thực nhận về tay của người lao động (Net)
...
# ==============================================================================
# 4. PHẦN NÚT BẤM KÍCH HOẠT VÀ HIỂN THỊ KẾT QUẢ GIAO DIỆN
# ==============================================================================
if st.button("🧮 Tính Thuế & Nhận Kết Quả", type="primary"):
    ...
    # Hiển thị các chỉ số tài chính vĩ mô bằng thẻ Metric
    ...
    # Văn bản thuyết minh cấu trúc luồng tiền thu nhập
    st.markdown(f"""
    ...
    * **Thu nhập tính thuế (đưa vào bảng lũy tiến):** `{res['assessable_income']:,.0f} VND`
    """)
    # Xử lý rẽ nhánh hiển thị dựa trên kết quả phát sinh thuế
    if res['tax'] > 0:
